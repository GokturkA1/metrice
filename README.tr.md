...
- **Dinamik Sürüm Senkronizasyonu:** Sunucu karşılama kimliği (`sshServerVersion`), `package.json` ile dinamik senkronize edilir (`SSH-2.0-Metrice_2.7.7`) ve `SSH_SERVER_VERSION` ortam değişkeni ile tamamen maskelenebilir.
...
| `sshServerVersion` | `SSH_SERVER_VERSION` | `'SSH-2.0-Metrice_2.7.7'` | SSH sunucusu protokol kimlik dizgesi (Sürüm sistemi ile dinamik) |
