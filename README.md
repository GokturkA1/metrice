...
- Dinamik Sürüm Sistemi & Yapılandırılabilir Kimlik: Sunucu kimlik dizgesi (`sshServerVersion`) ve sistem sürümü merkezi sürüm sistemi (`src/version.js`) üzerinden `package.json` ile dinamik olarak senkronize edilir (varsayılan: `SSH-2.0-Metrice_2.4.6`), ortam değişkeni veya konfigürasyon üzerinden özelleştirilebilir.
...
| `sshServerVersion` | `SSH_SERVER_VERSION` | `'SSH-2.0-Metrice_2.4.6'` | SSH sunucusu protokol kimlik dizgesi (Sürüm sistemi ile dinamik) |
...
Sistem bütünlüğü `tests/` klasöründeki üç kapsamlı test süiti (toplam 109 test) ile doğrulanır:
...
node tests/mesh.test.js      # 1. P2P-Mesh, AutoNAT, Rendezvous, PROXY ve Transit Spesifikasyon Süiti (77 Test)
